  ...
  async createLike(currentLikeBox) {
    try {
      const response = await axios.post(
        universityData.root_url + "/wp-json/heart/v1/manageLike",
        { doctorId: currentLikeBox.getAttribute("data-doctor") }
      );
  ...
  async deleteLike(currentLikeBox) {
    try {
      const response = await axios({
        url: universityData.root_url + "/wp-json/heart/v1/manageLike",
        method: "delete",
